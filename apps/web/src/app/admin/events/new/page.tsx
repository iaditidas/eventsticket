...
import { Plus, Trash2, Calendar, MapPin, CheckCircle, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Configure event details and ticket pricing tiers.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-card p-6 rounded-2xl border border-slate-800 space-y-6">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">Banner Image URL (Optional)</label>
            <input
              type="url"
              value={bannerImage}
        ...
        <div className="pt-6 border-t border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-white text-lg">Ticket Categories</h3>
            <button
              type="button"
              onClick={addCategory}
              className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-indigo-400 flex items-center space-x-1"
            >
          ...
          <div className="space-y-3">
            {categories.map((cat, i) => (
              <div key={i} className="grid grid-cols-12 gap-3 items-center bg-slate-900/60 p-3 rounded-xl border border-slate-800">
                <input
                  type="text"
                  value={cat.name}
                  onChange={(e) => updateCategory(i, 'name', e.target.value)}
                  placeholder="Category Name"
                  className="col-span-5 px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white"
                />
                <input
                  type="number"
                  value={cat.price}
                  ...
                  placeholder="Price $"
                  className="col-span-3 px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white"
                />
                <input
                  type="number"
